...
Due to a coding issue with outputs from MR-Hevo functions, the \acr{SE}s were not successfully retrieved. 95% \acr{CI}s were retrieved as intended. To aid comprehension and comparison of below results, it was decided to display an estimated \acr{SE} for MR-Hevo, calculated as $\frac{(Upper \space CI - Lower \space CI)}{3.92}$, i.e. approximating the bootstrap distribution as normally distributed as expected per \acr{CLT}. However, the reader should note that, whilst \acr{CI}s for MR-Hevo are accurate, MR-Hevo \acr{SE}s are approximate only.
...
no_causal_Hevo_mean_est_S1 <- no_causal_sim_summ_tib$Hevo_Av %>% filter() %>% mean() %>% signif(2)
no_causal_Hevo_mean_est_S2 <- no_causal_sim_summ_tib$Hevo_Av %>% mean() %>% signif(2)
no_causal_Hevo_mean_est_S3 <- no_causal_sim_summ_tib$Hevo_Av %>% mean() %>% signif(2)
