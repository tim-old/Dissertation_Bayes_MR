# Load packages
library(tidyverse)
library(TwoSampleMR)
library(rstan)
library(here)

# Load function scripts
source(here("MSc_Thesis_Split", "Script", "simulation_functions.R"))

# Set number of datasets used
n <- 100

# Load data
no_causal_10k_point0_scen1_models <- readRDS(file = here("MSc_Thesis_Split", "Data", "Simulated_Datasets", "no_causal_10k_point0_scen1_models.rds"))
no_causal_10k_point1_scen1_models <- readRDS(file = here("MSc_Thesis_Split", "Data", "Simulated_Datasets", "no_causal_10k_point1_scen1_models.rds"))
no_causal_10k_point2_scen1_models <- readRDS(file = here("MSc_Thesis_Split", "Data", "Simulated_Datasets", "no_causal_10k_point2_scen1_models.rds"))
no_causal_10k_point3_scen1_models <- readRDS(file = here("MSc_Thesis_Split", "Data", "Simulated_Datasets", "no_causal_10k_point3_scen1_models.rds"))

# Generate rows
no_causal_10k_point0_scen1 <- get_summary_MR_tib_row(no_causal_10k_point0_scen1_models[1:n])
saveRDS(no_causal_10k_point0_scen1, file = here("MSc_Thesis_Split", "Data", "Summary_Tables", "no_causal_10k_point0_scen1_row.rds"))

no_causal_10k_point1_scen1 <- get_summary_MR_tib_row(no_causal_10k_point1_scen1_models[1:n])
saveRDS(no_causal_10k_point1_scen1, file = here("MSc_Thesis_Split", "Data", "Summary_Tables", "no_causal_10k_point1_scen1_row.rds"))

no_causal_10k_point2_scen1 <- get_summary_MR_tib_row(no_causal_10k_point2_scen1_models[1:n])
saveRDS(no_causal_10k_point2_scen1, file = here("MSc_Thesis_Split", "Data", "Summary_Tables", "no_causal_10k_point2_scen1_row.rds"))

no_causal_10k_point3_scen1 <- get_summary_MR_tib_row(no_causal_10k_point3_scen1_models[1:n])
saveRDS(no_causal_10k_point3_scen1, file = here("MSc_Thesis_Split", "Data", "Summary_Tables", "no_causal_10k_point3_scen1_row.rds"))

# Combine rows
no_causal_10k_scen1_sim_summ_tib <- bind_rows(no_causal_10k_point0_scen1,
                                              no_causal_10k_point1_scen1,
                                              no_causal_10k_point2_scen1,
                                              no_causal_10k_point3_scen1)

# Save
saveRDS(no_causal_10k_scen1_sim_summ_tib, file = here("MSc_Thesis_Split", "Data", "Summary_Tables", "no_causal_10k_scen1_sim_summ_tib.rds"))


