...
test_plot_causal_0_errors <- plot_GY_GX(test_plot_tib_causal_0_errors, 
                                        plot_title = "Causal Effect Not Present")
...
test_plot_causal_0_inval <- plot_GY_GX(test_plot_tib_causal_0_inval, plot_title = "No Causal Effect")
...
test_plot_causal_1_inval <- plot_GY_GX(test_plot_tib_causal_1_inval, plot_title = "Causal Effect Present") +
  theme(axis.title.y = element_blank())
...
```{r test-plot-causal-unbal}


# Causal effect not present
set.seed(1701)
sim_test_data_causal_0_unbal <- simulate_MR_data(n_participants = 10000,
                                                 n_instruments = 100,
                                                 n_datasets = 1,
                                                 prop_invalid = 0.2,
                                                 causal_effect = FALSE,
...
test_plot_tib_causal_0_unbal <- extract_models(sim_test_data_causal_0_unbal)[[1]]

test_plot_causal_0_unbal <- plot_GY_GX(test_plot_tib_causal_0_unbal, plot_title = "No Causal Effect")

# Causal effect present
set.seed(1701)
sim_test_data_causal_1_unbal <- simulate_MR_data(n_participants = 10000,
                                                 n_instruments = 100,
                                                 n_datasets = 1,
                                                 prop_invalid = 0.2,
                                                 causal_effect = TRUE,
...
test_plot_tib_causal_1_unbal <- extract_models(sim_test_data_causal_1_unbal)[[1]]

test_plot_causal_1_unbal <- plot_GY_GX(test_plot_tib_causal_1_unbal, plot_title = "Causal Effect Present") +
  theme(axis.title.y = element_blank())
...
The variable phi represents additional pleiotropic effects of each invalid instrument when the InSIDE assumption (INstrument Strength Independent of Direct Effect) is not satisfied. The InSIDE assumption states that the gene:exposure association is not correlated with the pleiotropic path gene:outcome path of any invalid genetic instruments. This assumption can be violated if e.g.:
...
#                                    phi_max = 0.2){        
  ...
  # https://mr-dictionary.mrcieu.ac.uk/term/r-squared/#:~:text=The%20R%2Dsquared%20can%20also,and%20sample%20size%20of%20the
  output_tib_row <- results_tib %>% 
    summarise(N = n_datasets,
              Prop_Invalid = prop_invalid,
              F_Stat = R2_stat * n_participants / n_instruments,
              R2_stat = R2_stat,
              WME_Av = mean(WME_est),
              WME_SE = mean(WME_se),
...
# first element of each list element
# map(test_tib_summ_MR_data, first) %>% 
#   map(., first)

#test_tib_summ_MR_models[[1]]
