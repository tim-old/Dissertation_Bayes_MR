...
```{r test-plot-alpha, warning=FALSE}
...
```{r summary-MR-tib-row-fn, warning=FALSE}
...
get_summary_MR_tib_row <- function(n_participants = 1000,          # update to 10,000 for final
  ...
  for(dataset in 1:N_datasets){
    ...
    # https://pmc.ncbi.nlm.nih.gov/articles/PMC6424604/
  ...
  # https://academic.oup.com/ije/article/48/3/728/5251908 Appendix 2 for F-statistic
  
  output_tib_row <- results_tib %>% 
    summarise(N = N_datasets,
              Prop_Invalid = prop_invalid,
              F_stat = NA,
              #F_stat = ((n_participants - n_instruments - 1)/n_instruments) * (R2_stat/(1-R2_stat)), 
              R2_stat = NA,
              WME_Av = mean(WME_est),
              WME_SE = mean(WME_se),
...
#mr_weighted_median(b_exp = a_h_vect,b_out = g_h_vect,se_exp = se_a_h_vect,se_out = se_g_h_vect)

a_h <- as.data.table(test_plot_tib_hevo$coeff_G_X) 

se_a_h <- as.data.table(test_plot_tib_hevo$coeff_G_X_SE)

g_h <- as.data.table(test_plot_tib_hevo$coeff_G_Y)

se_g_h <- as.data.table(test_plot_tib_hevo$coeff_G_X_SE)
#names(se_g_h) <-  c("Instrument", "Value")


test_mod <- run_mrhevo.sstats(alpha_hat = a_h[, V1],
                              se.alpha_hat = se_a_h[, V1],
                              gamma_hat = g_h[, V1],
                              se.gamma_hat = se_g_h[, V1],
                              fraction_pleio = 0.2,
                              model.dir = here(),
...
a_h_tib <- test_plot_tib_hevo$coeff_G_X

se_a_h_tib <- test_plot_tib_hevo$coeff_G_X_SE

g_h_tib <- test_plot_tib_hevo$coeff_G_Y

se_g_h_tib <- test_plot_tib_hevo$coeff_G_X_SE
#names(se_g_h) <-  c("Instrument", "Value")


test_mod_tib <- run_mrhevo.sstats(alpha_hat = a_h_tib,
                              se.alpha_hat = se_a_h_tib,
                              gamma_hat = g_h_tib,
                              se.gamma_hat = se_g_h_tib,
                              fraction_pleio = 0.2,
                              model.dir = here(),
                              #priorsd_theta = priorsd_theta
) %>%
...
a_h_vect <- as.double(test_plot_tib_hevo$coeff_G_X)

se_a_h_vect <- as.double(test_plot_tib_hevo$coeff_G_X_SE)

g_h_vect <- as.double(test_plot_tib_hevo$coeff_G_Y)

se_g_h_vect <- as.double(test_plot_tib_hevo$coeff_G_X_SE)
#names(se_g_h) <-  c("Instrument", "Value")


test_mod_vect <- run_mrhevo.sstats(alpha_hat = a_h_vect,
                              se.alpha_hat = se_a_h_vect,
                              gamma_hat = g_h_vect,
                              se.gamma_hat = se_g_h_vect,
                              fraction_pleio = 0.2,
                              model.dir = here()
) %>%
  summary()
...
```{r system-time-testing, include=FALSE}
# Standard set-up for RStan

tib_time <- system.time(run_mrhevo.sstats(alpha_hat = a_h_tib,
                                          se.alpha_hat = se_a_h_tib,
                                          gamma_hat = g_h_tib,
                                          se.gamma_hat = se_g_h_tib,
                                          #fraction_pleio = 0.2,
...
dt_time <- system.time(run_mrhevo.sstats(alpha_hat = a_h[, V1],
                                         se.alpha_hat = se_a_h[, V1],
                                         gamma_hat = g_h[, V1],
                                         se.gamma_hat = se_g_h[, V1],
                                         fraction_pleio = 0.2,
...
vect_time <- system.time(run_mrhevo.sstats(alpha_hat = a_h_vect,
                              se.alpha_hat = se_a_h_vect,
                              gamma_hat = g_h_vect,
                              se.gamma_hat = se_g_h_vect,
                              fraction_pleio = 0.2,
...
cat("Vector: ", vect_time)

```

```{r test-hevo-2, warning=FALSE}
...
#test_tib_summ_MR[test_tib_summ_MR$Hevo_pval < 0.05, ]

test_hevo_list <- list()

# for(n in 1:50){
# test_plot_tib_hevo <- extract_models(sim_test_data_hevo)[[n]]
# 
# 
# test_hevo_list[[n]] <-  mr_weighted_median(b_exp = a_h_vect,b_out = g_h_vect,se_exp = se_a_h_vect,se_out = se_g_h_vect)$pval
# 
# }
# 
# length(test_hevo_list[test_hevo_list < 0.05])
```



```{r test-summary-MR-tib-row-fn, warning=FALSE, message=FALSE, cache=TRUE}



set.seed(14101583)
get_tib_MR_summ_causal_10k_10pc <-  get_summary_MR_tib_row(n_participants = 10000,
                                            n_instruments = 25,
                                            n_datasets = 1000,
                                            prop_invalid = 0.1,
                                            beta_val = 0.1,
