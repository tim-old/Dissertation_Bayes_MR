...
citations_search_sample %>% 
  ...
  fontsize(8, i = c(1:4), j = c(1:11), part = "header") %>% 
...
citations_reanalysis_summ_tib <- readRDS(here("MSc_Thesis_Split", "Data", "Summary_Tables", "citations_reanalysis_summ_tib.rds")) %>% tibble() %>% mutate(      across(Study_Ref, ~str_replace_all(., "\\[|\\]", "")))
...
Of the ten studies, MR-Hevo exhibited a favourable sensitivity versus \acr{WME} in `r causal_Hevo_better_n` (`r causal_Hevo_better_pc`%). For both MR-Hevo and \acr{WME} methods, causal report rate generally increased with an increasing proportion of invalid instruments, and also as assumption violations progressively presented greater data variability and bias across scenarios 1 to 3.
