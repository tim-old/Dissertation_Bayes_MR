...
A total of 110 abstracts and 54 full texts were screened to identify the 10 studies included [`r md_text(citations_search_sample_refs)`]; these are summarised in Table \@ref(tab:citation-search-sample-display). The flow diagram of study screening and selection is presented in Figure \@ref(fig:screening-flow).
...
```{r post-hoc-fn, include=FALSE}

get_post_hoc_data <- function(n_participants_x = as.integer(), 
                              n_participants_y = as.integer(),
  ...
  n_participants_x_list <- list()
  n_participants_y_list <- list()
  ...
  n_participants <- n_participants_x + n_participants_y
  ...
  sample_1_ref <- 1:n_participants_x        
  

  # create vector of participant indices for
  # participants used for estimating gene:exposure coefficient
  sample_2_ref <- (n_participants_x+1):(n_participants) # 2 sample MR
  ...
  for(n in 1:n_datasets){
    ...
    ifelse(rand_error == TRUE,
           ...
           U_epsilon_vect <- rep(0, n_participants))
    ...
    n_participants_x_list[[n]] <- n_participants_x
    n_participants_y_list[[n]] <- n_participants_y
  ...
  combined_list <- list(U = U_list,         # Estimates 
                        ...
                        n_participants_x = n_participants_x_list, # Inputs
                        n_participants_y = n_participants_y_list, 
...
```{r post-hoc-data, include=FALSE}

# Set parameters
cites_exposure_n <- citations_search_sample$Exposure_N_Participants %>% mean() %>% round(0)
cites_outcome_n <- citations_search_sample$Outcome_N_Participants %>% mean() %>% round(0)
cites_participants_n <- cites_exposure_n + cites_outcome_n
cites_instrument_n <- citations_search_sample$n_Instruments %>% mean() %>% round(0)
cites_beta_val <- citations_search_sample %>% 
  mutate(Beta = if_else(Reported_WME_Measure == "Beta",
                        Reported_WME_Causal_Effect,
                        log(Reported_WME_Causal_Effect))) %>% 
  pull(Beta) %>% 
  mean() %>% 
  round(2)

# Simulate data
causal_cites_means_point1_scen1_data <- get_simulated_MR_data(n_participants = cites_participants_n,
                                                              n_instruments = cites_instrument_n,
                                                              n_datasets = 10,
                                                              prop_invalid = 0.1,
                                                              causal_effect = TRUE,
                                                              balanced_pleio = TRUE,
                                                              InSIDE_satisfied = TRUE,
                                                              rand_error = TRUE,      # remove random errors, for testing
                                                              beta_val = cites_beta_val,         # size of causal effect
                                                              allele_freq_min = 0.1,  # frequency of effect allele 0.01/0.99
                                                              allele_freq_max = 0.9,  #?0.4/0.6
                                                              seed = 14101583)

```

```{r post-hoc-models, include=FALSE}

causal_cites_means_point1_scen1_models <- get_models(causal_cites_means_point1_scen1_data)
