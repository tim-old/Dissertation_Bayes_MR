...
extract_models_XY <- function(sim){
  ...
  for(dataset in 1:length(sim$X)){
    ...
    #ds <- as.data.frame(c(X,Y, Instruments_X, Instruments_Y, alpha, gamma, phi))
    
    # Model for gene:exposure
    #X_lm <- lm(X ~ 0 + Instruments_X, data = ds)
    X_lm <- lm(X ~ 0 + Instruments_X)
    coeff_X_G_vect <- coef(summary(X_lm))[1:(ncol(Instruments_X)), 1]
    SE_coeff_X_G_vect <- coef(summary(X_lm))[1:(ncol(Instruments_X)), 2]

    # Model for gene:outcome
    Y_lm <- lm(Y ~ 0 + Instruments_Y)
    coeff_Y_G_vect <- coef(summary(Y_lm))[1:(ncol(Instruments_Y)), 1] 
    SE_coeff_Y_G_vect <- coef(summary(Y_lm))[1:(ncol(Instruments_Y)), 2]
...
mr_weighted_median(b_exp = test_plot_tib_causal_0$coeff_X_G,
                   b_out = test_plot_tib_causal_0$coeff_Y_G,
                   se_exp = test_plot_tib_causal_0$coeff_X_G_SE, 
                   se_out = test_plot_tib_causal_0$coeff_Y_G_SE)
...
get_summary_MR_tib_row <- function(n_participants = 1000,          # update to 10,000 for final
  ...
  for(dataset in 1:N_datasets){
    ...
    WME_results <- mr_weighted_median(b_exp = model_list[[dataset]]$coeff_X_G,
                                      b_out = model_list[[dataset]]$coeff_Y_G,
                                      se_exp = model_list[[dataset]]$coeff_X_G_SE,
                                      se_out = model_list[[dataset]]$coeff_Y_G_SE)
...
```{r test-summary-MR-tib-fn, warning=FALSE, cache=TRUE}

set.seed(1583)
get_summary_MR_tib_row(n_participants = 10000,
#sim_test_data_multi <- wme_model_sim(n_participants = 10000,
                                     n_instruments = 25,
                                     n_datasets = 100,
                                     prop_invalid = 0.3,
                                     beta_val = 0.1,
...
#sim_test_model_multi <- extract_models_XY(sim_test_data_multi)



#get_summary_MR_tib_row(sim_test_model_multi)
