...
wme_model_sim <- function(n_participants = as.integer(), 
  ...
  for(n in 1:n_datasets){
    ...
    Yi_vect <-  G_mat[1:n_participants, ] %*% alpha_vect + 
                (beta * Xi_vect) + 
                Ui_vect[1:n_participants, ] + 
                Y_epsilon_vect
...
extract_models_XY <- function(sim){
  ...
  for(dataset in 1:length(sim$X)){
    ...
    Xi_vect <- coef(summary(X_lm))[2:(ncol(Instruments_X) + 1), ]
...
sim_test_data_causal_0 <- wme_model_sim(n_participants = 10000,
                                        ...
                                        InSIDE_satisfied = TRUE,
                                        rand_error = FALSE,
                                        beta_val = 100,                 # size of causal effect
                                        allele_freq_min = 0.01,         # frequency of effect allele
                                        allele_freq_max = 0.99,
                                        gamma_min = 0.03,               # size of pleiotropic effects on exposure
                                        gamma_max = 0.1,
...
test_plot_tib_causal_1 %>% 
  ...
  geom_text(aes(x = 0.1, # labels with gradient (causal effect estimate)
                y = 0.1, 
                label = paste0("beta == ", Gradient)), # beta hat
            parse = TRUE) +
