...
extract_models_XY <- function(sim){
  ...
  #for(dataset in 1:2){
    ...
    coeff_X_G_vect <- coef(summary(X_lm))[2:(ncol(Instruments) + 1), 1]
    SE_coeff_X_G_vect <- coef(summary(X_lm))[2:(ncol(Instruments) + 1), 2]

    # Model for gene:outcome
    Y_lm <- lm(Y ~ Instruments)
    coeff_Y_G_vect <- coef(summary(Y_lm))[2:(ncol(Instruments) + 1), 1]
    SE_coeff_Y_G_vect <- coef(summary(Y_lm))[2:(ncol(Instruments) + 1), 2]
    
    output_list[[dataset]] <- as_tibble(list(dataset = dataset,
                                             Instrument = c(1:ncol(Instruments)),
...
set.seed(1701)
sim_test_data_causal_0 <- wme_model_sim(n_participants = 100000,
                                     n_instruments = 25,
                                     n_datasets = 1,
                                     prop_invalid = 0.1,
                                     causal_effect = FALSE,
...
set.seed(1701)
sim_test_data_causal_1 <- wme_model_sim(n_participants = 100000,
                                     n_instruments = 25,
                                     n_datasets = 1,
                                     prop_invalid = 0.1,
                                     causal_effect = TRUE,
...
test_plot_tib_causal_0 %>% 
  select(coeff_X_G, coeff_Y_G) %>% 
  plot(.,
       main = "No Causal Effect Present")

test_plot_tib_causal_1 %>% 
  select(coeff_X_G, coeff_Y_G) %>% 
  plot(.,
       main = "Causal Effect Present")
